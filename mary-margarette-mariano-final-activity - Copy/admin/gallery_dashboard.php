...
<!-- carousel management--> 
	  <table class="table-borderless" >
        ...
        <tbody>
		<?php
			$Load="SELECT * FROM imagegallery";
			$result =$conn->query($Load);
			if($result->num_rows>0) {
			while($rowUsers=$result->fetch_assoc()) {
			?>
			<form action="../config/imagegallery.php" method="POST">
			<input type="hidden" name="galleryID" value="<?=$rowUsers['galleryID']?>"> 
            <tr>
			<td><?=$rowUsers['gallery']?></td>
			<td> <img src="../uploadsGallery/<?=$rowUsers['gallery']?>" width="100" height="100"> </td>
				
		 <td><button formaction="../config/deleteGallery.php" onclick="return confirm('Delete this gallery image?')" style="padding: 10px; border-style: none; background-color: inherit;"type="submit" ><i class="fa fa-trash"></i></button> </td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
     	<form action="../config/uploadGallery.php" method="post" enctype="multipart/form-data">
